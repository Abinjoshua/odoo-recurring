...
class BillingSchedulePortal(http.Controller):
    ...
    @http.route('/delete-billing-schedule/<int:bill_id>', type='http', auth='user', website=True)
    ...
    @http.route('/delete-request/billing-schedule/<int:bill_id>', type='http', auth='public', website=True)
    def delete_request_billing_schedule(self, bill_id, **kwargs):
        billing_schedule = request.env['recurring.billing.schedule'].sudo().browse(bill_id)
        if request.env.user.has_group('recurring_subscription.group_subscription_manager'):
            ...
            billing_schedule.write({'state': 'delete_requested'})
        return request.redirect('/view/billing-schedules')
